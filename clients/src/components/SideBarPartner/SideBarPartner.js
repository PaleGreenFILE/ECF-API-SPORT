...
const SideBarPartner = () => {
  ...
  const Menus = [
    ...
    { key: 3, title: "Contact", icon: <AiOutlineMail />, link: "/partenaire/contact" },
    {
      key: 2,
      title: "Planning",
  ...
  return (
    <div className="flex">
      ...
      >
        ...
        <div className="flex">
          <img
            className={` h-10 bg-white block float-left mr-2 duration-500 ${open && "rotate-[360deg]"
              }`}
            src="https://bit.ly/3IUtNVM"
            alt="logo"
          />
          <div
            className={` text-yellow-300 origin-left font-medium text-sm mt-2 ${!open && "scale-0"
              }`}
          >
            FitPark Fitness
          </div>
          ...
          <div className="text-gray-100 text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:text-blue-600 hover:bg-yellow-300 rounded-md mt-12 duration-300">
            <span className=" flex text-2xl  float-left">
              ...
              <button
                ...
                className={`text-base ml-3 font-medium flex-1 duration-200 ${!open && "hidden"
                  }`}
              >
                Se deconnecter
              </button>

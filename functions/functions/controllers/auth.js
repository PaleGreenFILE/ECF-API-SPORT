...
export const signinAdmin = async (req, res, next) => {
  ...
  try {
    const admin = await db.query("SELECT * FROM clients WHERE email=$1", [
      email,
    ]);
    const partner = await db.query("SELECT * FROM partenaires WHERE email=$1", [
      email,
    ]);
    const structure = await db.query(
      "SELECT * FROM partenaires WHERE email=$1",
      [email]
    );
    if (!admin.rows.length & !partner.rows.length & !structure.rows.length)
      return next(createError(404, "Email non inscrit"));
